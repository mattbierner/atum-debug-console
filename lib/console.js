...
*/
;;;;
require(["knockout-2.2.1", "parse/parse", "nu/stream", "atum/interpret", "atum/compute", "atum/compute/context", "atum/builtin/impl/global", "atum/builtin/operations/global", "atum/operations/object", "atum/semantics/semantics", "atum/debug/debugger", "ecma/lex/lexer", "ecma/parse/parser"], (function(ko, parse, stream, interpret, compute, context, global, global_operations, object, semantics, atum_debugger, lexer, parser) {
    var map = Function.prototype.call.bind(Array.prototype.map);
    var reduce = Function.prototype.call.bind(Array.prototype.reduce);
    var get = (function(p, c) {
        return p[c];
    });
    var globalCtx = interpret.complete(compute.sequence(global.initialize(), global_operations.enterGlobal(), compute.computeContext), context.ComputeContext.empty, (function(x) {
        return x;
    }), (function(x) {
        return x;
    }));
    var printBindings = (function(d, record) {
        var envRecord;
        if (record.ref) {
            var obj = d.getValue(record, (function(x, ctx) {
                return x;
            }), (function(x, ctx) {
                return x;
            }));
        ...
        return Object.keys(record).reduce((function(p, c) {
            p.push(({
                "name": c,
                "value": d.getValue(record[c].value, (function(x, ctx) {
                    return x;
                }), (function(x, ctx) {
                    return x;
                }))
    ...
    var printEnvironments = (function(d, ctx) {
        var environments = [];
        if (ctx.userData) {
            var environment = d.getValue(ctx.userData.lexicalEnvironment, (function(x, ctx) {
                return x;
            }), (function(x, ctx) {
                return x;
            }));
            do {
                ...
                (environment = d.getValue(environment.outer, (function(x, ctx) {
                    return x;
                }), (function(x, ctx) {
                    return x;
                })));
    ...
    var run = (function(input, ok, err) {
        ...
        return interpret.complete(semantics.programBody(semantics.sourceElements(ast.body)), globalCtx, ok, err);
    });
    var runContext = (function(input, ctx, ok, err) {
        try {
            var ast = parser.parse(input);
            return interpret.complete(semantics.programBody(semantics.sourceElements(ast.body)), ctx, ok, err);
        } catch (e) {
            return err(e, null);
        }
    ...
    var AtumObject = (function(d, x, ctx) {
        var self = this;
        var value = d.getValue(x, (function(x, ctx) {
            return x;
        }), (function(x, ctx) {
            return x;
        }));
    ...
    var ConsoleViewModel = (function() {
        ...
        (this.environments = ko.computed((function() {
            return (self.debug() ? printEnvironments(self.debug(), self.debug().ctx) : []);
        })));
        (this.stack = ko.computed((function() {
            return ((self.debug() && self.debug().ctx.userData) ? ko.utils.arrayMap(self.debug().ctx.userData.metadata.stack, (function(x) {
                return ({
                    "name": (x.func ? self.debug().run(object.get(x.func, "name"), (function(x) {
                        return x.value;
                    })) : "")
                });
            })) : []);
    ...
    $((function() {
        ...
        $("button#debug-button").button().click((function() {
            ...
            try {
                ...
                var ctx = globalCtx;
                model.debug(atum_debugger.Debugger.create(p, ctx, out.write, errorOut.write));
